...
class khane_irani_FAQ_Settings extends khane_irani_Settings_Tab {

    public function __construct() {
        parent::__construct('faq', 'سوالات پرتکرار');
    }
        ...
        <form method="post" action="options.php" id="zarin-settings-form">
            ...
            <table class="form-table">
                ...
                $this->render_field(array(
                    'type' => 'image',
                    'id' => 'faq_section_image',
                    'label' => 'تصویر ستون اول',
                    'default' => '',
                    'description' => 'تصویر نمایش داده شده در ستون اول سکشن سوالات متداول'
                ));
                
                $this->render_faq_repeater(array(
                    'id' => 'faq_items',
                    'label' => 'مدیریت سوالات پرتکرار',
                    'description' => 'سوال/پاسخ، تصویر (اختیاری)، شاخص (کارت بزرگ)، وضعیت و ترتیب.'
                ));
                ?>
            </table>
            <?php submit_button('ذخیره سوالات پرتکرار'); ?>
            <input type="hidden" name="active_tab" value="faq" />
        </form>
        <?php
    }
    ...
    private function get_default_items() {
        return array(
            array(
                ...
                'answer' => 'روی دکمه افزودن به سبد خرید کلیک کنید و مراحل خرید را ادامه دهید.',
                'author' => '',
                'image' => '',
                'is_featured' => '1',
                'is_active' => '1',
    ...
    public function render_faq_repeater($args) {
        $items = $this->get_option($args['id'], $this->get_default_items());
        if (empty($items) || !is_array($items)) { $items = $this->get_default_items(); }
        ?>
        <tr>
        ...
        jQuery(function($){
            ...
            $('.zs-faq-manager').on('click','.add-faq',function(){ $('.zs-faq-manager .faq-items').append(tpl(idx++)); });
            $('.zs-faq-manager').on('click','.remove-faq',function(){ $(this).closest('.faq-item').remove(); });
            $('.zs-faq-manager').on('click','.upload-image-button',function(){
                const button=$(this), input=button.siblings('.faq-image-input'), preview=button.siblings('.image-preview'), removeBtn=button.siblings('.remove-image-button');
            ...
            $('.zs-faq-manager').on('click','.remove-image-button',function(){ $(this).siblings('.faq-image-input').val(''); $(this).siblings('.image-preview').empty(); $(this).hide(); });
        });
        </script>
        <style>
