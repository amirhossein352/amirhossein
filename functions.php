...
/**
 * رفع مشکل صفحه تشکر - نسخه قطعی
 */
add_action('template_redirect', 'force_thankyou_page_display', 9999);
function force_thankyou_page_display() {
    // فقط در صفحه تشکر
    ...
    // فقط برای سفارش‌های موفق
    if (!$order->has_status('failed')) {
        // غیرفعال کردن همه فیلترها و اکشن‌هایی که ممکن است صفحه را تغییر دهند
        remove_all_filters('template_include');
        remove_all_filters('template_redirect');
        
        // اطمینان از بارگذاری تمپلیت درست
        add_filter('template_include', function($template) {
            if (is_wc_endpoint_url('order-received')) {
                // پیدا کردن تمپلیت صفحه تشکر
                $template_path = WC()->plugin_path() . '/templates/checkout/thankyou.php';
...
/**
 * جلوگیری از کش صفحه تشکر
 */
add_action('send_headers', 'no_cache_thankyou_page');
function no_cache_thankyou_page() {
    if (is_wc_endpoint_url('order-received')) {
        // غیرفعال کردن کش
        nocache_headers();
        
        // هدرهای اضافی برای جلوگیری از کش
        header('Cache-Control: no-cache, no-store, must-revalidate, max-age=0');
        header('Cache-Control: post-check=0, pre-check=0', false);
        header('Pragma: no-cache');
        header('Expires: Sat, 26 Jul 1997 05:00:00 GMT');
        
        // اطمینان از کد وضعیت 200
        if (!headers_sent()) {
            status_header(200);
        }
    }
...
/**
 * رفع مشکل تکراری بودن محتوا
 */
add_action('wp', 'prevent_duplicate_checkout_content', 1);
function prevent_duplicate_checkout_content() {
    if (is_wc_endpoint_url('order-received')) {
        // حذف اکشن‌های صفحه پرداخت
